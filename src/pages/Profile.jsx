...
import { useRooms } from "../context/RoomsContext"; // context for managing rooms

export default function Profile() {
  const navigate = useNavigate();
  const [user, setUser] = useState(null);
  const { cancelBooking } = useRooms(); // direct function from context
          ...
          <div className="space-y-4">
            {user.bookings.map((booking) => (
              ...
              >
                ...
                <button
                  onClick={() => handleCancelBooking(booking.id, booking.room.id)}
